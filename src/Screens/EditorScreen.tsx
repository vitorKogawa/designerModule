...
function EditorScreen(props: any) {
  ...
  useEffect(() => {
    ...
    async function populate() {
      //pegando a conexão entre os nós source (origem) e target(destino) de cada nó que vai compor o tabuleiro
      const connectionsResult = await getConnections();
      const connRes = await connectionsResult.json();

      //Capturando dados nos nós (nodes) e do jogo
      const gamesResult = await getNodes();
      const result = await gamesResult.json();
      result.game.nodes.map((item: any, index: number) => {
